...
if ($_POST["postal_code"]) {
  $updateStmt = $conn->prepare("UPDATE payment_methods SET postal_code = :value WHERE  id= :id");
  $updateStmt->bindParam(':value', $_POST["postal_code"]);
  $updateStmt->bindParam(':id', $payment_id, PDO::PARAM_INT);
  $updateStmt->execute();
  Header('Location: '.$_SERVER['PHP_SELF'].'?payment_id='.$_GET["payment_id"]);
}
...
if ($_POST["is_active"]) {
  ...
  $updateStmt->bindParam(':value', $is_active, PDO::PARAM_INT);
  $updateStmt->bindParam(':id', $payment_id, PDO::PARAM_INT);
  $updateStmt->execute();
  Header('Location: '.$_SERVER['PHP_SELF'].'?payment_id='.$_GET["payment_id"]);
}
